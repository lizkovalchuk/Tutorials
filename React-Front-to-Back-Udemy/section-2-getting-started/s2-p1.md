...
- `const` and `let` are new in ES6 and they were created to block level scoping.
...
- It is recommended to use `const` whenever possible and only use `let` when you know that you re-assign that vairable or need to initialize something and add value to it later.
...
After great thing about ES6 is the template literal, which makes using the concatinating operator unnecessary.
...
`.map` is similar to `forEach` however it returns an array and you change each item in the array. In the below example, were going to remove the 's' on every fruit in the fruits array and store them in a new array.
...
`.filter` is used to return an array with things filtered out. `.filter` is often used in state managers like Redux or context APIs where you want to delete when something from the state, you want to "filter" it out.
...
Modules allow us to use vairables, functions, classes and objects from other files. In order to do this is export whatever you need to import into other files.
