...
- We still need the function `onDeleteClick` attached to our delete icon. Though instead of altering the state within the `Contact.js` component, which is how it has been working so far, we need to attached `onDeleteClick` with `context.js`, as we need to affect the state in there.
...
5. Inside the `swtich` statement, we are going to want to evaluate the type, and since we are passing an object to the second parameter of `action` in `reducer`, we use not notation to access the `type`. The `type` will be a capatilized string.
6. In your `switch` statement, create a case for `DELETE_CONTACT`. If that case passes, return the state with the deleted contact which you identity by `id` with `filter()`. Use the spread operator to make a copy of the immutable state.
...
- In order to be alve to call the dispatch, we have to pass that into `onDeleteClick` as a parameter.
